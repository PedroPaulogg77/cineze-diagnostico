...
function AcessoContent() {
  ...
  async function handleSubmitEmail(e: React.FormEvent) {
    ...
    try {
      ...
      setErroMsg("Erro de conexão. Verifique sua internet.")
      setEstado("email")
    }
  }
    ...
    <>
      <div className="flex justify-center mb-10">
        <div className="relative w-28 h-28 flex items-center justify-center">
          ...
          <svg className="w-16 h-16 animate-spin text-blue-400" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-20" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
            <path className="opacity-80" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
          </svg>
        </div>
...
function TelaEmail({
  ...
  return (
    <>
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white mb-2 tracking-tight">Crie seu Acesso</h2>
        <p className="text-gray-400 text-sm leading-relaxed">
          Pagamento confirmado! Crie uma senha para acessar seu painel do diagnóstico.
        </p>
      </div>

      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          <label htmlFor="email" className="block text-sm font-semibold text-gray-300 mb-2">Email da compra</label>
          <input
            id="email"
            type="email"
            value={emailInput}
        ...
        <div>
          <label htmlFor="password" className="block text-sm font-semibold text-gray-300 mb-2">Crie uma senha (mínimo 6 caracteres)</label>
          <input
            id="password"
            type="password"
            value={passwordInput}
        ...
        {erroMsg && (
          <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
            <p className="text-red-400 text-sm font-medium">{erroMsg}</p>
          </div>
        )}
